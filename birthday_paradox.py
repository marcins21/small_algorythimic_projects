...
for i in range(how_many_simulations):
    number_of_lists = [[] for i in range(how_many_people)]
    for j in range(how_many_people):
        #adding random days and month [[month,day] ... ]
        random_day = random.choice(DAYS)
        random_month = random.choice(MONTHS)
        number_of_lists[j].append(random_month)
        number_of_lists[j].append(random_day)

    #information that the program is not stuck
    if i % 1000 == 0:
        print(i,"/",how_many_simulations,"Simulations Completed")

    #getting value of the list of '100' and '0'
    same_birthday_result = find_same_birthdays(number_of_lists)

    ################################################################################################################
    #cheking if '100' is in the list then adding '100' to another list else adding '0'
    #because if 100 EXIST the list then add '100' to antoher list :
    # that means that we hit same birthday in the group of people
    #
    # if 100 DO NOT EXIST this mean that we didn't hit same (it's possbile) birthday in a group of people
    ###############################################################################################################
...
# We're calculating ratio of HITTING or NOT HITTING the same birthday in the group of people we can recieve
# We can HIT or NOT HIT --> 100% or 0%
